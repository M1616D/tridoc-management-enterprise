...
const UI = {
    ...
    updateDashboardStats() {
        document.getElementById('kpi-total-docs').textContent = STATE.stats.totalDocs;
        document.getElementById('kpi-total-storage').textContent = formatBytes(STATE.stats.totalSize);
        document.getElementById('kpi-processed-pages').textContent = STATE.stats.totalDocs * 3;
        document.getElementById('storage-used-label').textContent = formatBytes(STATE.stats.totalSize);
    ...
    renderCharts() {
        ...
        STATE.activityChart = new Chart(ctxActivity, {
            ...
            data: {
                ...
                datasets: [{
                    label: 'Indexed Files',
                    data: [12, 19, 15, 25, 32, 28, STATE.stats.totalDocs + 10],
                    borderColor: '#9eff2f',
        ...
        STATE.formatChart = new Chart(ctxFormat, {
            ...
            data: {
                ...
                datasets: [{
                    data: [Math.max(STATE.stats.totalDocs, 1), 2, 1],
            ...
            tbody.innerHTML += `
                <tr class="hover:bg-dark-700/50 transition-colors duration-200">
                    <td class="px-4 py-3 font-medium text-white flex items-center gap-3">
                        <i class="fa-solid fa-file-lines text-brand"></i> ${doc.filename}
                    </td>
                    <td class="px-4 py-3"><span class="px-2.5 py-1 rounded-lg bg-dark-700 text-xs font-semibold text-gray-300">${doc.category}</span></td>
                    <td class="px-4 py-3 text-gray-400">${formatBytes(doc.size)}</td>
                    <td class="px-4 py-3 text-gray-400">${formatDate(doc.uploadDate)}</td>
        ...
        results.forEach(res => {
            container.innerHTML += `
                <div class="bg-dark-800 border border-dark-700 p-6 rounded-2xl shadow-card hover:shadow-glow transition-shadow duration-200">
                    ...
                    <p class="text-sm text-gray-300 font-mono bg-dark-900 p-3 rounded-xl border border-dark-700 mt-3">...${res.snippet || res.fullText.substring(0, 200)}...</p>
                </div>
            `;
        });
    },
    ...
    setupSettings() {
        ...
        document.getElementById('setting-autocommit').checked = settings.autoIndex;
        document.getElementById('setting-snippet-len').value = settings.maxSnippetLen;
        document.getElementById('setting-ocr').checked = settings.ocrEnabled;
        ...
        document.getElementById('setting-storage-limit').value = STATE.rootStorage;
        // Update icon
        const icon = document.getElementById('theme-icon');
        if (icon) {
    ...
    setupEventListeners() {
        ...
        // Theme toggle (handled only here, not inline)
...
const App = {
    async handleFiles(filePaths) {
        UI.showToast('Parsing files...', 'info');
        UI.closeUploadModal();
        ...
        if (STATE.currentView === 'documents') UI.renderDocumentsList();
        UI.showToast('Files uploaded and indexed!', 'success');
    },
    async search(query) {
